...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (typeof hasPermission === "function") {
        if (!hasPermission("create_transactions")) {
            await swalError("คุณไม่มีสิทธิ์แก้ไขรายการความเคลื่อนไหว");
    ...
    if (!id) {
        await swalError("ไม่พบรหัสรายการที่ต้องการแก้ไข");
        window.location.href = "/transactions/transactions.html";
        return;
    }
    ...
    try {
        ...
        console.error(err);
        await swalError("เกิดข้อผิดพลาดในการโหลดข้อมูลเดิม");
        window.location.href = "/transactions/transactions.html";
    }
    ...
    saveBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            if (res.ok) {
                await swalSuccess("แก้ไขรายการสำเร็จ");
                window.location.href = "/transactions/transactions.html";
            } else {
