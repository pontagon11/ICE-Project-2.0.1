...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (typeof hasPermission === "function") {
        if (!hasPermission("edit_bom")) {
            await swalError("คุณไม่มีสิทธิ์แก้ไขสูตรการผลิต");
    ...
    if (!bomId) {
        await swalError("ไม่พบรหัสสูตรการผลิต (BOM ID)");
        window.location.href = "/bom/bom.html";
        return;
    }
    ...
    async function loadBom() {
        try {
            ...
            if (!res.ok) {
                await swalError(data.message || "โหลดข้อมูล BOM ไม่สำเร็จ");
                window.location.href = "/bom/bom.html";
                return;
            }
    ...
    addMaterialBtn.addEventListener("click", async () => {
        ...
        if (!matId || isNaN(usageQty) || usageQty <= 0) {
            await swalWarning("กรุณาเลือกวัตถุดิบและระบุจำนวนการใช้");
            return;
        }

        // ตรวจสอบรายการซ้ำ (เช็กจาก ID)
        if (bomDetails.some(d => String(d.mat_id) === String(matId))) {
                await swalWarning("วัตถุดิบนี้มีอยู่ในสูตรแล้ว หากต้องการเปลี่ยนจำนวนให้ลบแล้วเพิ่มใหม่");
            return;
        }
    ...
    saveBomBtn.addEventListener("click", async () => {
        if (bomDetails.length === 0) {
            await swalWarning("สูตรการผลิตต้องมีวัตถุดิบอย่างน้อย 1 รายการ");
            return;
        }

        const { isConfirmed } = await swalConfirm("ยืนยันการแก้ไขสูตรการผลิตนี้?");
        ...
        try {
            ...
            if (res.ok) {
                await swalSuccess("แก้ไข BOM เรียบร้อยแล้ว");
                window.location.href = "/bom/bom.html";
            } else {
                const result = await res.json();
                await swalError(result.message || "แก้ไขไม่สำเร็จ");
