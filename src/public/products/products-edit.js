document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (typeof hasPermission === "function") {
    if (!hasPermission("edit_products")) {
      await swalError("คุณไม่มีสิทธิ์แก้ไขสินค้านี้");
  ...
  if (!productId) {
    await swalError("ไม่พบรหัสสินค้า (Product ID)");
    window.location.href = "/products/products.html";
    return;
  }
...
async function loadProduct(id) {
  try {
    ...
    if (!res.ok) {
      await swalError(data.message || "ไม่พบข้อมูลสินค้า");
      window.location.href = "/products/products.html";
      return;
    }
    ...
    await swalError("โหลดข้อมูลล้มเหลว");
  }
}
...
function setupButtons(id) {
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    if (!name || isNaN(price) || !status) {
      await swalWarning("กรุณากรอกข้อมูลให้ครบและถูกต้อง");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        await swalSuccess("แก้ไขข้อมูลสินค้าเรียบร้อยแล้ว");
        window.location.href = "/products/products.html";
      } else {
