...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (typeof hasPermission === "function") {
        if (!hasPermission("view_stock")) {
            await swalError("คุณไม่มีสิทธิ์เข้าถึงระบบจัดการสต็อก");
...
async function loadApprovedPO() {
    ...
    if (typeof hasPermission === "function" && !hasPermission("receive_purchase")) {
        await swalError("คุณไม่มีสิทธิ์ดำเนินการรับของเข้าสต็อค");
        return;
    }
...
async function receivePO(id, btn) {
    const { isConfirmed } = await swalConfirm("ยืนยันการรับสินค้า/วัสดุเข้าสต็อค?");
    ...
    try {
        ...
        if (res.ok) {
            await swalSuccess("อัปเดตสต็อคเรียบร้อยแล้ว");
            loadApprovedPO();
        } else {
