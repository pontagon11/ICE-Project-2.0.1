...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (typeof hasPermission === "function") {
        if (!hasPermission("edit_employee")) {
            await swalError("คุณไม่มีสิทธิ์แก้ไขข้อมูลพนักงาน");
    ...
    if (!id) {
        await swalError("ไม่พบรหัสพนักงาน (Employee ID)");
        window.location.href = "/employees/employee.html";
        return;
    }
...
async function loadEmployee(id) {
    try {
        ...
        if (!res.ok) {
            await swalError("ไม่พบข้อมูลพนักงานในระบบ");
            window.location.href = "/employees/employee.html";
            return;
        }
        ...
        await swalError("เกิดข้อผิดพลาดในการโหลดข้อมูลพนักงาน");
    }
}
...
function setupButtons(id) {
    ...
    editBtn?.addEventListener("click", async () => {
        ...
        if (!fname || !lname) {
            await swalWarning("กรุณาระบุชื่อและนามสกุลพนักงาน");
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                await swalSuccess("อัปเดตข้อมูลพนักงานสำเร็จ");
                window.location.href = "/employees/employee.html";
            } else {
