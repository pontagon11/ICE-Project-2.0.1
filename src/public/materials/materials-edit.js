document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!matId) {
        await swalError("ไม่พบรหัสวัตถุดิบที่ต้องการแก้ไข");
        window.location.href = "/materials/materials.html";
        return;
    }
    ...
    try {
        ...
        if (!res.ok) {
            await swalError(m.message || "โหลดข้อมูลวัตถุดิบไม่สำเร็จ");
            window.location.href = "/materials/materials.html";
            return;
        }
    ...
    saveBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            if (res.ok) {
                await swalSuccess("แก้ไขข้อมูลวัตถุดิบสำเร็จ");
                window.location.href = "/materials/materials.html";
            } else {
    ...
    cancelBtn.addEventListener("click", async () => {
        const { isConfirmed } = await swalConfirm("คุณต้องการยกเลิกการแก้ไขใช่หรือไม่? ข้อมูลที่กรอกไว้จะไม่ถูกบันทึก", "ยกเลิก");
        if (isConfirmed) {
            window.location.href = "/materials/materials.html";
        }
    });
