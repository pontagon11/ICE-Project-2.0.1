...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (typeof hasPermission === "function") {
    if (!hasPermission("view_qc")) {
      alert("❌ คุณไม่มีสิทธิ์เข้าถึงหน้ารายงาน QC");
  ...
  document.getElementById("addBtn")?.addEventListener("click", () => {
    window.location.href = "qc-add.html";
  });
});
...
async function handleQC(id, result) {
  ...
  try {
    ...
    if (res.ok) {
      alert(`บันทึกผล ${result.toUpperCase()} สำเร็จ ✅`);
      loadQC(); // โหลดตารางใหม่เพื่ออัปเดตสถานะ
    } else {
      const data = await res.json();
      alert(data.message || "การบันทึกผลล้มเหลว");
    }
  } catch (err) {
