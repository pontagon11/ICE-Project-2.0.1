...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (typeof hasPermission === "function") {
        if (!hasPermission("view_employee")) {
            alert("❌ คุณไม่มีสิทธิ์เข้าถึงข้อมูลส่วนนี้");
    ...
    if (addBtn) {
        if (!hasPermission("create_employee")) {
            ...
            addBtn.onclick = () => window.location.href = "employee-add.html";
        }
    }
...
async function deleteEmployee(id) {
    if (!hasPermission("edit_employee")) {
        alert("คุณไม่มีสิทธิ์ลบข้อมูลพนักงาน");
        return;
    }

    if (!confirm("⚠️ ยืนยันการลบข้อมูลพนักงานคนนี้? \nการลบอาจส่งผลกระทบต่อประวัติการทำรายการต่างๆ")) return;
