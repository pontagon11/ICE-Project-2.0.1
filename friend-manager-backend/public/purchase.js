...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        if (!hasPermission("view_purchase") && !permissions.includes("view_purchase")) {
            alert("❌ คุณไม่มีสิทธิ์เข้าถึงรายการใบสั่งซื้อ");
        ...
        const profileImg = document.getElementById("profileImg"); // ดึง Element รูปมา
...
async function receivePO(id, btn) {
    if (!confirm("ยืนยันการรับวัตถุดิบเข้าคลัง?")) return;
    ...
    try {
        ...
        if (res.ok) {
            alert("รับวัตถุดิบเข้าคลังสำเร็จ! ✅");
            loadPurchase(); // โหลดใหม่เพื่ออัปเดตสถานะ
        } else {
...
document.getElementById("addBtn")?.addEventListener("click", () => {
    window.location.href = "purchase-add.html";
});