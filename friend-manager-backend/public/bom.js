...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (typeof hasPermission === "function") {
    if (!hasPermission("view_bom")) {
      alert("❌ คุณไม่มีสิทธิ์เข้าถึงหน้ารายการสูตรการผลิต");
...
async function deleteBOM(bomId) {
    if (!confirm("⚠️ คุณต้องการลบสูตรการผลิต (BOM) ทั้งหมดของสินค้านี้ใช่หรือไม่? \nการลบนี้จะไม่สามารถย้อนคืนได้")) return;
    ...
    try {
        ...
        if (res.ok) {
            alert("ลบสูตรการผลิตเรียบร้อยแล้ว ✅");
            loadBoms(); // โหลดใหม่
        } else {
...
function setupAddButton() {
    ...
    addBtn.onclick = () => window.location.href = "bom-add.html";
}

// Helper functions สำหรับการเปลี่ยนหน้า
function editBom(bomId) {
    window.location.href = `bom-edit.html?id=${bomId}`;
}